  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center px-4">
      <div className="max-w-md w-full text-center space-y-8">
        ...
        {/* Status icon */}
        <div className="relative mx-auto flex items-center justify-center h-28 mb-4">
          {isWorking && (
            icon ? (
              <div className="relative w-24 h-24 flex items-center justify-center">{icon}</div>
            ) : (
              <div className="relative w-24 h-24">
                ...
                <div
                  ...
                  style={{ borderColor: accentColor, borderTopColor: 'transparent', animationDuration: '1.5s' }}
                />
                
                {/* Inner flipping card */}
                <div className="absolute inset-0 flex items-center justify-center" style={{ perspective: 1000 }}>
                  <motion.div
                    className="w-10 h-14 rounded-md border-[2px] bg-white shadow-sm flex flex-col items-center justify-center p-1"
                    style={{ borderColor: accentColor }}
                    animate={{ rotateY: [0, 180, 180, 0] }}
                    transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
                  >
                    <div className="w-full flex justify-end">
                      <div className="w-1.5 h-1.5 rounded-full opacity-40 mb-1" style={{ backgroundColor: accentColor }} />
                    </div>
                    <div className="w-5 h-5 rounded-[2px] opacity-20 mb-1" style={{ backgroundColor: accentColor }} />
                    <div className="w-full h-1 rounded-full opacity-30 mt-auto" style={{ backgroundColor: accentColor }} />
                  </motion.div>
                </div>
              </div>
            )
          ...
          {isComplete && (
            <CheckCircle2 className="w-24 h-24 text-green-500 origin-center" />
          )}
          {isError && (
            <AlertCircle className="w-24 h-24 text-red-500 origin-center" />
          )}
        </div>
